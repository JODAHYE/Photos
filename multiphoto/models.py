...
class MultiPhoto(models.Model):
    ...
    lover = models.ManyToManyField(settings.AUTH_USER_MODEL, null=True, related_name='like_photos')
...
class Image(models.Model):
    ...
    file = ProcessedImageField(
        ...
        processors=[processors.Transpose(), ResizeToFit(width=720)],  # 처리할 작업 목록
        format='JPEG',  # 저장 포맷(확장자)
        options={'quality': 70},  # 저장 포맷 관련 옵션 (JPEG 압축률 설정)
    )
